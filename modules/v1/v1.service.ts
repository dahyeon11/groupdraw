...
import { teatime, teatime_has_tgroup, tgroup, Prisma } from '@prisma/client'
...
export class V1Service {
  ...
  async findOne(): Promise<teatime>{
    return await this.prisma.teatime.findUnique({
      where: {
        id: 1
      },
      include: {
  ...
  async generateNewGroup(newGroupDto: newGroupDto): Promise<string[][]> {
    const crew = dummy.crew.slice(0); //직원 배열 복제
    ...
    const numberOfSmallGroup =
      numberOfGroup - (numberOfCrew % newGroupDto.limit); //한명이 부족한 그룹 수
    ...
    for (let groupIndex = 1; groupIndex <= numberOfGroup; groupIndex++) {
      ...
      //팀장 뽑기
      const randomCrewIdx = await crypto.randomInt(numberOfCrew);
      ...
      for (
        ...
        newMemberIndex < numberOfGroup;
        newMemberIndex++
      ) {
        //직원 수와 그룹 단위를 계산해 한명이 부족한 팀 구성
        if (
          newMemberIndex === newGroupDto.limit - 1 &&
          groupIndex >= numberOfSmallGroup
        ) {
          newGroup.push('X');
          continue;
        }

        const weightArr = generateWeightArr(rawWightArr); //확률로 계산된 가중치 배열
        const newMember = weightedSample(crew, weightArr); //가중치를 적용하여 팀원 샘플
